...
class AIEngine:
    ...
    def choose_move(self, board: Board, player: int = AI):
        ...
        if not valid:
            return 0, {"depth": 0, "nodes": 0, "nps": 0.0, "time": 0.0, "score": 0}

        ordered_seed = sorted(valid, key=lambda c: abs(c - CENTER_COL))

        try:
            for depth in range(1, self.max_depth + 1):
                score, move = self._negamax(board, depth, -math.inf, math.inf, AI, 0, ordered_seed)
                self.best_score, self.best_move = score, move
    ...
    def _negamax(self, board: Board, depth: int, alpha: float, beta: float,
                 player: int, ply: int, order_hint: List[int]) -> Tuple[float, Optional[int]]:
        ...
        for col in moves:
            ...
            try:
                ...
                if winner_cells:
                    ...
                    child_score, _ = self._negamax(board, depth - 1, -beta, -alpha, -player, ply + 1, order_hint)
                    score = -child_score
            finally:
